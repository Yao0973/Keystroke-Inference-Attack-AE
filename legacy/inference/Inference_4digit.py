...
def main():
    ...
    DATA_FILE = 'full_test_data_4digit.csv'  # ← your.*data
    # ===================
    ...
    # verify column count: 1(pin) + 4(ts) + 4*5(features) = 25
    ...
    for idx, row in df.iterrows():
        ...
        for i in range(PIN_LEN):
            ...
            # ✅ key:normalize original features
            # Fix:remove.*call，because.*are already.*arrays
        ...
        if not USE_KINEMATIC:
            ...
            dp = [{} for _ in range(PIN_LEN)]
            parent = [{} for _ in range(PIN_LEN)]
            ...
            # Backtrack Path
            final_candidates = {d: dp[PIN_LEN-1][d] for d in candidate_sets[PIN_LEN-1] if d in dp[PIN_LEN-1]}
            if not final_candidates:
                pred_pin = mlp_pred_pin
            else:
                last = max(final_candidates, key=final_candidates.get)
                path = [last]
                for pos in range(PIN_LEN-1, 0, -1):
    ...
    print("\n" + "=" * 60)
    print("🔐 4-digit PIN inference results")
