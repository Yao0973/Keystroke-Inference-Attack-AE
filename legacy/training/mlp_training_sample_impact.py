...
class MorphologyMLP(nn.Module):
    def __init__(self, input_dim=5, num_classes=10):
        super().__init__()
        # Network structure extracted from original file
...
def train_and_evaluate(train_loader, X_test_tensor, y_test_tensor, device):
    ...
    # Use class weights from original file
...
def run_experiment():
    ...
    for n in sample_sizes:
        accuracies = []
        print(f"Testing training sample size N={n}...")
        for run in range(n_runs):
            ...
            for cls in range(10):
                ...
                if len(cls_indices) < n:
                     indices.extend(cls_indices) # Take all if not enough samples
                else:
        ...
        # Save normalization parameters during training (inferred code)
